...
class FullVisualActorModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.v_fc = torch.nn.Sequential(
            torch.nn.Linear(num_outputs, 64),
            torch.nn.ReLU(),
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        self.v_vals = self.v_fc(logits)  # Save value function for later use

        return logits, state

    def value_function(self):

        return torch.reshape(self.v_vals, [-1])
