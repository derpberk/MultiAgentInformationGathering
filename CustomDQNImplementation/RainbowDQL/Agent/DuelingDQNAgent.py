...
class MultiAgentDuelingDQNAgent:
	...
	def select_action(self, dict_state: dict):

		actions_dict = {agent_id: self.individual_select_action(ind_state) for agent_id, ind_state in dict_state.items()}

		return actions_dict

	def process_action(self, action: dict, done: dict) -> Tuple[dict, dict, dict]:
		"""Take an action and return the response of the env."""

		actions_for_ready_agents = {agent_id: action[agent_id] for agent_id in action.keys() if not done[agent_id]}
		next_state, reward, done, _ = self.env.step(actions_for_ready_agents)
	...
	def train(self, episodes):
		...
		for episode in trange(1, int(episodes) + 1):
			...
			while not done['__all__']:
				...
				score += np.mean(np.sum(list(reward.values())))  # The mean reward among the agents
